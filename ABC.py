...
def run_simulation(optimizer, objective_func, function_name, num_trials, max_cycles, tolerance, dimension):
    all_results = []
    ...
    for trial in range(num_trials):
        ...
        print(f"ABC\t{function_name}\t{result['best_fitness']:.10f}\t{dimension}\t{position_str}")
...
if __name__ == '__main__':
    ...
    if args.trials > 1:
        ...
        print("="*80)
        position_str = "\t".join([f"{pos:.10f}" for pos in result['best_position']])
        print(f"ABC\t{args.function.replace('_', ' ').title()}\t{result['best_fitness']:.10f}\t{args.dimension}\t{position_str}")
