...
class McCormick_PSO:
    ...
    def mccormick_function(self, x):
        x1, x2 = x[:, 0], x[:, 1]
        fitness = np.sin(x1 + x2) + (x1 - x2)**2 - 1.5 * x1 + 2.5 * x2 + 1
        return fitness
    ...
    def train(self, max_iterations=1000):
        self.fitness = self.mccormick_function(self.particles)  # Ilk uygunluk degerleri
        ...
        while self.iteration < max_iterations and self.gbest[-1] >= 1e-6:  # Belirli bir uygunluk degerine ulasana kadar devam et
            self.iteration += 1
            self.fitness = self.mccormick_function(self.particles)  # Her iterasyonda uygunluk hesapla
...
# McCormick fonksiyonu icin PSO'yu calistirma
pso_object = McCormick_PSO(-1.5, 4, 50, 2, 2, 2)  # 2 boyutlu McCormick fonksiyonu
