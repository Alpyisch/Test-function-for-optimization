...
class PSOOptimizer:
    def __init__(self, lower_bound, upper_bound, particle_count, dimension, c1=2, c2=2):
        ...
        self.particles = np.random.uniform(
            ...
            size=(self.particle_count, self.dimension)
        )

        self.velocities = np.random.uniform(-1, 1, (self.particle_count, self.dimension))
...
def main():
    parser = argparse.ArgumentParser(description='Particle Swarm Optimization for Various Mathematical Functions')
    ...
    parser.add_argument('--tolerance', type=float, default=1e-6, help='Tolerance for convergence')
    args = parser.parse_args()

    # Define bounds for all functions
    bounds = {
        'sphere': (-10, 10),
    ...
    # Define dimension requirements for functions
    ...
    if args.function not in dimension_requirements:
        raise ValueError(f"Dimension requirements for function '{args.function}' are not defined.")

    required_dimension = dimension_requirements[args.function]
    if args.dimension != required_dimension:
        raise ValueError(f"Function '{args.function}' requires dimension {required_dimension}, but {args.dimension} was provided.")
    ...
    result = optimizer.optimize(args.function, max_iterations=args.max_iterations, tolerance=args.tolerance)
    end_time = time.time()
