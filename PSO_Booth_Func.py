...
class Booth_PSO:
    ...
    def booth_function(self, x1, x2):
        fitness = (x1 + 2 * x2 - 7) ** 2 + (2 * x1 + x2 - 5) ** 2
        return fitness
    ...
    # Eğitim fonksiyonu
    def train(self):
        self.fitness = self.booth_function(self.particles[:, 0], self.particles[:, 1])
        ...
        while self.gbest[-1] >= 1e-6:  # Belirli bir uygunluk değerine ulaşılana kadar devam et
            self.iteration += 1
            self.fitness = self.booth_function(self.particles[:, 0], self.particles[:, 1])  # Her iterasyonda uygunluk hesapla
...
# Booth fonksiyonu için PSO'yu çalıştırma
pso_object = Booth_PSO(-10, 10, 50, 2, 2, 2)  # 2 boyutlu Booth fonksiyonu
