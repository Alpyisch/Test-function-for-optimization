...
class Eggholder_PSO:
    ...
    def eggholder_function(self, x1, x2):
        fitness = -(x2 + 47) * np.sin(np.sqrt(np.abs(x1 / 2 + (x2 + 47)))) - x1 * np.sin(np.sqrt(np.abs(x1 - (x2 + 47))))
        return fitness
    ...
    # Eğitim fonksiyonu
    def train(self):
        self.fitness = self.eggholder_function(self.particles[:, 0], self.particles[:, 1])
        ...
        self.iteration = 0
        while self.gbest[-1] >= -959.64:  # Eggholder için hedef değere ulaşılana kadar devam et
            self.iteration += 1
            self.fitness = self.eggholder_function(self.particles[:, 0], self.particles[:, 1])  # Her iterasyonda uygunluk hesapla
...
# Eggholder fonksiyonu için PSO'yu çalıştırma
pso_object = Eggholder_PSO(-512, 512, 50, 2, 2, 2)  # 2 boyutlu Eggholder fonksiyonu
